            ...
            for dest in range(V):
                ...
                if num_edges > 1:
                    for adjacent in range(V):
                        if (
                            graph[src][adjacent] != float("inf")
                            and adjacent not in [src, dest]
                            and memo[adjacent][dest][num_edges - 1] != float("inf")
                        ):
                            memo[src][dest][num_edges] = min(
                                memo[src][dest][num_edges],
                                graph[src][adjacent]
                                + memo[adjacent][dest][num_edges - 1],
                            )
